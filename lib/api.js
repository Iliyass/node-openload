...
class Openload {
  ...
  get request(){
    ...
    return request.bind(null, config.api_url);
  }
  /**
   ...
   */
  getAccountInfo() {
    return this.request(`${this.locationPrefix}/account/info`, 'GET', this.config);
  }
  ...
  getDownloadTicket(file) {
    ...
    return this.request(`${this.locationPrefix}/file/dlticket`, 'GET', conf);
  }
  ...
  getDownloadLink(obj) {
    if (obj !== null && typeof obj === 'object') {
      ...
      return this.request(`${this.locationPrefix}/file/dl`, 'GET', obj);
    }
  ...
  getFileInfo(file) {
    ...
    if (conf.file.length < 50) {
      ...
      return this.request(`${this.locationPrefix}/file/info`, 'GET', conf);
    }
  ...
  deleteFile(file) {
    ...
    if (conf.file.length < 50) {
      ...
      delete conf.file;
      ids.forEach(id => promises.push(this.request(`${this.locationPrefix}/file/delete`, 'GET', {
        login: conf.login,
        key: conf.key,
        file: id,
      })));
  ...
  listFolder(folder) {
    ...
    return this.request(`${this.locationPrefix}/file/listfolder`, 'GET', conf);
  }
  ...
  remoteUpload(obj) {
    ...
    if (obj !== null && typeof obj === 'object') {
      ...
      return this.request(`${this.locationPrefix}/remotedl/add`, 'GET', conf);
    }
  ...
  remoteUploadStatus(obj) {
    ...
    return this.request(`${this.locationPrefix}/remotedl/status`, 'GET', conf);
  }
   ...
   * @param {string} obj.contentType (optional) When using a Buffer as upload, append content type
  ...
  upload(obj, cb) {
    ...
    if (obj !== null && typeof obj === 'object') {
      ...
      const upl = new Upload(`${this.locationPrefix}/file/ul`, conf, cb);
      return upl.upload();
    }
  ...
  /**
   * Gets the thumbnail of given file id:
   * https://openload.co/api#file-splash
   * @param {string} file a file ID to return thumbnail for
   * @returns A promise containing thumbnail about file
   */
  getSplashImage(file) {
    const conf = this.config;
    conf.file = file;
    ...
    return this.request(`${this.locationPrefix}/file/getsplash`, 'GET', conf);
  }
}
